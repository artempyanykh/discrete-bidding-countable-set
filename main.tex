...
\begin{lemma}
  ...
  Тогда если существует ее сходящаяся по норме подпоследовательность $\{l_{k_n}\}$, то и сама последовательность $\{l_n\}$ сходится по норме.
\end{lemma}
...
\begin{proof}\underline{[Лемма~\ref{lower-bound:lemma:subseq-convergence}]}
  ...
  Тогда для любого $\varepsilon > 0$ существует $N$ такое, что для любых $j_f, j_g \geqslant N$ выполнено 
  $\norm{l_{j_f} - l_{j_g}} \leqslant \varepsilon$.
  В то же время для любых $m \geqslant n \geqslant N$ можно найти $j_q \leqslant n, j_p \geqslant m$, что будет выполнено
  \begin{gather*}
    \norm{l_m - l_n} =
    \sum_{s=0}^\infty s^2 |l_m^s - l_n^s| \leqslant
    \sum_{s=0}^\infty s^2 |l_{j_p}^s - l_{j_q}^s| =
    \norm{l_{j_p} - l_{j_q}} \leqslant \varepsilon.
  \end{gather*}
...
\begin{proof}\underline{[Лемма~\ref{lower-bound:lemma:decomp-convergence}]}
  ...
  \begin{equation*}
    s_n = \sum_{r=x+1}^n \sum_{l=0}^{x-1} \alpha_{l,r}(\p) \p^x(l, r).
  \end{equation*}
