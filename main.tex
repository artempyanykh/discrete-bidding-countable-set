...
\begin{gather}
  ...
  \p = p^x e^x + \sum_{r=\floor{x+1}}^\infty \sum_{l=0}^{\ceil{x-1}} \alpha_{l,r}(\p) p^x(l, r),\\
  \label{lower-bound:eq:prob-decomp-coeff}
  \alpha_{l,r}(\p) = (r-l) p^l p^r / \sum_{t=0}^{\ceil{x-1}} p^t (x-t),
\end{gather}
где $\floor{x}$ --- целая часть $x$, а $\ceil{x} = \floor{x} + 1$.
...
\begin{proof}\underline{[Лемма~\ref{lower-bound:lemma:decomp-convergence}]}
  ...
  \end{gather*}
  Отсюда в силу леммы~\ref{lower-bound:lemma:subseq-convergence} получаем, что ряд в разложении~\eqref{lower-bound:eq:prob-decomp-sum} также сходится к $\p$.
