...
\begin{definition}
  Стратегией первого игрока является последовательность ходов $\sigmav = (\sigma_1,
  \ldots, \sigma_n)$, где $\sigma_t: S \times I^{t-1} \rightarrow \Delta(I)$.
  Множество стратегий первого игрока обозначим $\FPStrategies$.
\end{definition}
...
\begin{definition}
  ...
  стратегий второго игрока обозначим $\SPStrategies$.
\end{definition}

Таким образом, первый игрок на каждом шаге игры рандомизирует свои действия в зависимости от состояния рынка $s$ и истории ставок.
...
Представим стратегию первого игрока в виде $\sigmav = (\sigma, \sigmav^i, i \in I)$, где $\sigma$~--- ход игрока на первом шаге, а $\sigmav^i$ --- стратегия в игре продолжительности $n-1$ в зависимости от ставки $i$ на первом шаге.
...
Тогда для функции выигрыша в игре $\G[n](\p)$ будет справедлива формула
\begin{equation}
...
\begin{equation*}
  ...
  \end{cases}
\end{equation*}
При использовании этой стратегии второй игрок делает ставку равную $k$ на первом шаге, а далее подражает инсайдеру.
...
Кроме того, хотя описание чистой стратегии $\tauv^k$ неосведомленного игрока повторяет описание из \cite{bib:domansky07}, стратегия $\tauv^*$ второго игрока зависит от $\beta$ в силу того, что от $\beta$ зависит выбор конкретной $\tauv^k$ в определении $\tauv^*$.
...
гарантировать себе первый игрок в игре $\G(\p)$.
\begin{lemma}
  \label{lower-bound:lemma:convex-combination}
  Пусть $\p_k$ --- распределение из $\Delta(S)$, $\sigmav_k$ --- стратегия первого игрока, которая гарантирует ему выигрыш $\Low[n](\p_k)$ в игре $\G[n](\p_k)$,
  ...
  \end{equation}
  гарантирует первому игроку выигрыш $\lambda \Low[n](\p_1) + (1-\lambda)
  \Low[n](\p_2)$.
\end{lemma}
...
Распространяя результаты \cite{bib:pyanykh16} на случай $\p^{k+\beta}(l, r)$, определим следующую стратегию первого игрока в игре $\G[\infty](\p)$.
Введем обозначение %
\begin{equation*}
  P(l,r) = \left\{
...
образом: если $\p = \p^l(l,r)$ или $\p = \p^r(l,r)$ первый игрок использует ставки
$l$ и $r$, соответственно, с вероятностью 1; иначе он использует
...
Использование первым игроком стратегии $\sigmav^*$ порождает случайное блуждание последовательности апостериорных вероятностей, изображенное на рисунке~\ref{fig:posterior-1}, которое в отличие от \cite{bib:domansky11} происходит по более широкому множеству и уже не является симметричным, кроме случая $\beta = 1/2$.
...
\begin{equation*}
  ...
  \right\}.
\end{equation*}
При $\p \in P'(l,r)$ первый ход стратегии $\xiv^*$ определяется следующим образом: если $\p = \p^l(l,r)$ или $\p = \p^r(l,r)$ первый игрок использует ставки $l$ и $r$, соответственно, с вероятностью 1; иначе он использует $\sigmak_k$ с параметрами
\begin{equation}
...
\begin{proof}\underline{[Лемма~\ref{lower-bound:lemma:convex-combination}]}
  ...
  \end{multline*}
  Получили, что стратегия $\sigmav_c$ обеспечивает первому игроку в игре $\G[n](\p)$ соответствующую выпуклую комбинацию гарантированных выигрышей в играх $\G[n](\p_1)$ и $\G[n](\p_2)$.
\end{proof}
...
\begin{proof}\underline{[Теорема~\ref{lower-bound:theorem}]}
  Для $\p \in P(l,r)$ определение стратегия $\sigmav^*$ аналогично определению оптимальной стратегии первого игрока из \cite{bib:pyanykh16} с заменой $0, m$ на $l, r$ соответственно.
