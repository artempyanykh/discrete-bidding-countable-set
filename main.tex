...
\newcommand{\p}{\ensuremath{\overline{p}}}
\newcommand{\q}{\ensuremath{\overline{q}}}
...
\begin{definition}
  ...
  \tau_n)$, где $\tau_t: I^{t-1} \rightarrow \Delta(J)$. Множество стратегий
  игрока 2 обозначим $\SPStrategies$.
\end{definition}
...
\begin{definition}
  ...
  \begin{equation*}
    \K(\p, \sigma, \tau) =
    \E_{(\p, \sigma, \tau)} \sum_{t=1}^n a^s(i_t, j_t),
  \end{equation*}
  где математическое ожидание берется по мере, индуцированной $\p$, $\sigma$ и
  $\tau$.
\end{definition}
Заданную таким образом игру обозначим $\G(\p)$.

\begin{definition}
  Если для некоторых $\sigma^* \in \FPStrategies,$ $\tau^* \in \SPStrategies$
  выполняется
  \begin{equation*}
    \sup_{\sigma \in \FPStrategies} \inf_{\tau \in \SPStrategies} \K(\p, \sigma, \tau) =
    \K(\p, \sigma^*, \tau^*) =
    \inf_{\tau \in \SPStrategies} \sup_{\sigma \in \FPStrategies} \K(\p, \sigma, \tau) = 
    \V(\p),
  \end{equation*}
  то игра $\G(\p)$ имеет значение $\V(\p)$, а стратегии $\sigma^*$ и $\tau^*$
  являются оптимальными.
\end{definition}

Следуя \cite{bib:domansky11}, опишем рекурсивную структуру игры $\G(\p)$.
Представим стратегию игрока 1 в виде $\sigma = (\sigma_1, \sigma^i, i \in I)$,
где $\sigma_1$ --- ход игрока на первом шаге, а $\sigma^i$ --- стратегия в игре
продолжительности $n-1$ в зависимости от ставки $i$ на первом шаге. Аналогично,
стратегию игрока 2 представим в виде $\tau = (\tau_1, \tau^i, \; i \in I)$.
%
...
\begin{equation*}
  \K[n](\p, \sigma, \tau) =
  \K[1](\p, \sigma_1, \tau_1) +
  \sum_{i \in I} q_i \K[n-1](\p^i, \sigma^i, \tau^i).
\end{equation*}
...
Следуя \cite{bib:domansky11} рассмотрим чистую стратегию $\tau^k$ игрока 2,
...
\begin{lemma}
  ...
  При применении стратегии $\tau^k$ в игре $\G(\p)$ игрок 2 в состоянии $s$
  гарантирует себе проигрыш не более
  \begin{gather*}
    h^s_n(\tau^k) = \sum_{t=0}^{n-1} (k-s-t-1+\beta)^+, \; s \leqslant k, \quad
    h^s_n(\tau^k) = \sum_{t=0}^{n-1} (s-k-t-\beta)^+, \; s \geqslant k.
  \end{gather*}
  Последовательность $\left\{ h^s_n(\tau^k), \; n = \overline{1, \infty}
  \right\}$ монотонна, ограничена сверху и сходится к %
  $h^s_\infty(\tau^k) = (s - k + 1 - 2\beta)(s-k)/2$.
\end{lemma}
...
\begin{theorem}
  ...
  \begin{equation*}
    \High(\p) = \min_{k \in J} \sum_{s \in S} p_s  h^s_\infty(\tau^k).
  \end{equation*}
  ...
  \end{equation*}
  Стратегия $\tau^*$, которая позволяет игроку 2 обеспечить данную оценку,
  состоит в применении $\tau^k$ при $\p \in \Lambda(k - 1 + \beta, k + \beta)$.
\end{theorem}
